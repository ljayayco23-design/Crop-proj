...
@section('title', 'RICEGUARD AI • User Accounts')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">All admin, technician, and farmer accounts on the system.</p>
        </div>
    ...
    {{-- ===================== ROLE SUMMARY CARDS ===================== --}}
    @php
        // Admin accounts are only ever visible to developer — a normal
        // admin's $allowedRoles never contains 'admin' (see
        // AdminUserController::allowedRolesForActor()), so the card is
        // dropped entirely rather than shown with a count of 0.
        $showAdminCard = in_array('admin', $allowedRoles, true);
        $visibleCardCount = $showAdminCard ? 3 : 2;
        $cardColClass = $visibleCardCount === 3 ? 'col-md-4' : 'col-md-6';
    @endphp
    <div class="row g-3 mb-4">
        @if($showAdminCard)
        <div class="{{ $cardColClass }}">
            <div class="card role-card role-card-admin h-100" data-role="admin" role="button" tabindex="0" onclick="selectRole('admin')">
                <div class="card-body d-flex align-items-center gap-3">
                    <div class="role-icon role-icon-admin"><i class="fas fa-user-shield"></i></div>
                    <div>
                ...
                <div class="card-footer role-card-footer d-flex justify-content-between align-items-center">
                    <span class="text-muted small">Total Admins</span>
                    <strong class="role-count" id="count-admin">{{ $admins->count() }}</strong>
                </div>
            </div>
        </div>
        @endif
    ...
    <div class="card uas-panel">
        <div class="card-body">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3 uas-toolbar">
                <div>
                    <h5 class="uas-title mb-1" id="table-title">Admin Accounts</h5>
                    <p class="uas-subtitle mb-0" id="table-subtitle">List of all admin accounts and their registered address.</p>
                </div>
            ...
            <div class="table-responsive">
                <table class="table uas-table mb-0 align-middle">
                    ...
                    <tbody id="users-table-body">
                        @foreach(['admin' => $admins, 'technician' => $technicians, 'farmer' => $farmers] as $roleKey => $collection)
...
@section('scripts')
<script>
    const roleLabels = { admin: 'Admin Accounts', technician: 'Technician Accounts', farmer: 'Farmer Accounts' };
    const subtitleLabels = {
        admin: 'List of all admin accounts and their registered address.',
    ...
    function updateRoleCounts() {
        ['admin', 'technician', 'farmer'].forEach(role => {
