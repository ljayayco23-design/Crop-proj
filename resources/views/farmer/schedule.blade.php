...
@php
    // ============================================================
    // This page is OFFLINE-FIRST: it keeps its own IndexedDB copy of
    // this farmer's schedule and syncs batched changes through
    // /schedule/sync whenever the device is online (see sync()/idb*
    // below and ScheduleController@sync). None of that changes here.
    //
    // Page chrome (sidebar / top nav / notifications / quick search /
    // profile panel / global chat widget) now comes entirely from
    // layouts.farmer, the same layout every other farmer page (e.g.
    // announcements) uses  this page used to duplicate all of that
    // itself with a second, out-of-sync mobile sidebar/burger
    // implementation (whose JS even pointed at a #farmerSidebar id
    // that didn't exist in its own markup), which is what caused the
    // nav icons to behave differently on phones. The service worker
    // is also already registered once by the layout, so the page no
    // longer registers it a second time.
    // ============================================================
    $cfg = [
        'uid'   => $uid,
        'csrf'  => csrf_token(),
        'urls'  => [
            'events' => route('farmer.schedule.events', [], false),
            'sync'   => route('farmer.schedule.sync', [], false),
            'sw'     => '/sw.js',
        ],
    ];
@endphp
...
.s-add{background:linear-gradient(#17a94d,#12923f);border:1px solid #2fd063;color:#fff;font-weight:600;border-radius:9px;padding:9px 18px;cursor:pointer;display:inline-flex;gap:8px;align-items:center;font-size:15px}
...
.s-pill i{width:8px;height:8px;border-radius:50%;background:#22c55e;display:inline-block}
.s-pill em{font-style:normal}
.s-pill.off i{background:#f59e0b}
.s-pill.sy i{background:#3b82f6}
...
.s-mg .d.t span,.s-mg .d.t:hover span{background:#10a55a;color:#fff;font-weight:600}
...
.s-addcal{margin-top:8px;padding:12px 0 0;border:0;border-top:1px solid var(--ln);background:none;color:#22c55e;display:flex;gap:10px;width:100%;cursor:pointer;font-size:14px;align-items:center}
...
.tn{background:#10a55a;color:#fff;border-radius:50%;min-width:26px;height:26px;display:inline-flex;align-items:center;justify-content:center;font-weight:600}
...
.s-empty button{background:none;border:0;color:#22c55e;cursor:pointer;padding:0;text-decoration:underline}
...
@media(max-width:767px){
  ...
  #btnAdd{position:fixed;right:16px;bottom:calc(18px + env(safe-area-inset-bottom));z-index:50;width:58px;height:58px;padding:0;border-radius:50%;justify-content:center;gap:0;font-size:0;box-shadow:0 10px 24px rgba(0,0,0,.55),0 0 0 5px rgba(34,197,94,.16)}
  ...
  .st-d.has .dot{background:#22c55e}
  .st-d.t{border-color:#10a55a}
  .st-d.a{background:#10a55a;border-color:#10a55a;color:#fff}
  .st-d.a small{color:#d1fae5}
  ...
  .s-g2 input,.s-g2 select,.s-g2 textarea{font-size:16px;padding:10px 12px}
    ...
    <div class="s-wrap">
        <header class="s-head">
            ...
            <div class="s-hr">
                <span class="s-pill" id="pill"><i></i><em>Synced</em></span>
        ...
        <div class="s-cols">
            ...
            <section>
                ...
                <div class="s-card s-up">
                    ...
                    <div class="s-tw">
                        <table class="s-tbl">
                            <thead><tr><th>Date &amp; Time</th><th>Type</th><th>Location</th><th>Technician</th><th>Status</th><th class="r">Actions</th></tr></thead>
...
<div class="s-mbg" id="modal" hidden>
    <form class="s-md" id="form" autocomplete="off" novalidate>
        ...
        <div class="s-g2">
            ...
            <label class="mf">Technician<input name="technician" maxlength="120" list="techList"></label>
        ...
        <div class="s-act">
            ...
            <button type="submit" class="s-add">Save</button>
        </div>
    </form>
</div>
...
const st = { events: new Map(), view: 'week', anchor: new Date(), mini: new Date(), tech: '', q: '', cal: { my: 1, team: 1, inspection: 1, treatment: 1, maintenance: 1, followup: 1, other: 1 }, showOther: false, ready: false, syncing: false, again: false, err: '' };

/* ---------- local storage (IndexedDB, one database per signed-in user) ---------- */
let db = null;
const idbOpen = () => new Promise((ok, no) => {
  const r = indexedDB.open('riceguard-schedule-' + S.uid, 1);
  r.onupgradeneeded = () => r.result.createObjectStore('events', { keyPath: 'uuid' });
  r.onsuccess = () => { db = r.result; ok(); };
  r.onerror = () => no(r.error);
});
const store = (mode) => db.transaction('events', mode).objectStore('events');
const req = (r) => new Promise((ok, no) => { r.onsuccess = () => ok(r.result); r.onerror = () => no(r.error); });
const idbAll = () => req(store('readonly').getAll());
const put = (r) => db ? req(store('readwrite').put(r)).catch(() => {}) : Promise.resolve();
const del = (id) => db ? req(store('readwrite').delete(id)).catch(() => {}) : Promise.resolve();

/* ---------- filtering ---------- */
const all = () => [...st.events.values()].filter((e) => !e.deleted);
...
function agendaHtml() {
  ...
  if (!list.length) return '<div class="ag-empty"><i class="fa-regular fa-calendar-check"></i><p>Nothing scheduled for this day.</p><button class="s-ghost" data-add>Add a schedule</button></div>';
...
function renderUp() {
  ...
  if (!list.length) {
    $('#upBody').innerHTML = !st.ready ? '' : '<tr><td colspan="6" class="s-empty">' + (all().length ? 'No upcoming schedules match your filters.' :
      'No schedules yet. Use Add Schedule, or <button id="btnSample">load a sample week</button>.') + '</td></tr>';
    return;
  }
...
function renderTech() {
  ...
  $('#selTech').innerHTML = '<option value="">All Technicians</option>' + t.map((x) => '<option' + (x === st.tech ? ' selected' : '') + '>' + esc(x) + '</option>').join('');
  $('#techList').innerHTML = t.map((x) => '<option value="' + esc(x) + '">').join('');
}

function pill() {
  const n = [...st.events.values()].filter((e) => e.dirty).length, p = $('#pill'), t = p.querySelector('em'), M = isM();
  if (!navigator.onLine) { p.className = 's-pill off'; t.textContent = M ? 'Offline' + (n ? ' \u00b7 ' + n : '') : 'Offline \u00b7 saved on this device' + (n ? ' (' + n + ' to sync)' : ''); }
  else if (st.syncing) { p.className = 's-pill sy'; t.textContent = 'Syncing\u2026'; }
  else if (st.err) { p.className = 's-pill off'; t.textContent = M ? 'Sign in to sync' : st.err; }
  else if (n) { p.className = 's-pill sy'; t.textContent = M ? n + ' to sync' : n + ' waiting to sync'; }
  else { p.className = 's-pill'; t.textContent = 'Synced'; }
}

function render() {
  renderTech(); renderMini(); renderStrip(); renderCals(); renderMain(); renderUp(); pill();
  $('#legend').innerHTML = Object.values(TYPES).map((t) => '<span><i class="c-' + t[1] + '"></i>' + t[0] + '</span>').join('');
}

/* ---------- create / edit / delete (always local first) ---------- */
async function save(rec) {
  rec.updated_at = Date.now(); rec.dirty = 1; rec.deleted = 0;
  st.events.set(rec.uuid, rec);
  await put(rec); render(); sync();
}
async function removeEv(id) {
  const r = st.events.get(id); if (!r) return;
  r.deleted = 1; r.dirty = 1; r.updated_at = Date.now();
  await put(r); render(); sync();
}

const SAMPLE = [
  [0, 'Field Inspection', 'inspection', 'North Field 3', '08:00', '09:30', 'Juan Dela Cruz', 'approved', 'blue'],
  [1, 'Weed Management', 'treatment', 'South Field 2', '10:00', '11:00', 'Maria Santos', 'approved', 'green'],
  [2, 'Follow-up Check', 'followup', 'West Field 2', '09:00', '10:30', 'Ana Reyes', 'pending', 'purple'],
  [2, 'Field Inspection', 'inspection', 'North Field 3', '14:00', '15:00', 'Juan Dela Cruz', 'approved', 'blue'],
  [3, 'Field Inspection', 'inspection', 'North Field 3', '08:00', '09:30', 'Juan Dela Cruz', 'approved', 'blue'],
  [3, 'Apply Fungicide', 'treatment', 'East Field 1', '09:00', '12:00', 'Maria Santos', 'pending', 'amber'],
  [4, 'Equipment Maintenance', 'maintenance', 'Drone #2', null, null, 'Roberto Garcia', 'approved', 'red'],
  [4, 'Repair Irrigation', 'maintenance', '', '11:00', '12:00', 'Roberto Garcia', 'pending', 'purple'],
  [4, 'Fertilizer Check', 'treatment', 'South Field 2', '14:00', '15:00', 'Maria Santos', 'approved', 'green'],
  [5, 'Field Inspection', 'inspection', 'North Field 3', '08:00', '09:30', 'Juan Dela Cruz', 'approved', 'blue'],
  [6, 'Maintenance', 'maintenance', 'Drone #1', '10:00', '12:00', 'Juan Dela Cruz', 'pending', 'purple'],
  [8, 'Field Inspection', 'inspection', 'West Field 4', '08:00', '09:30', 'Ana Reyes', 'approved', 'blue'],
  [9, 'Apply Fungicide', 'treatment', 'East Field 1', '09:00', '12:00', 'Maria Santos', 'pending', 'amber'],
  [10, 'Pump Maintenance', 'maintenance', 'South Field 2', '14:00', '15:00', 'Roberto Garcia', 'approved', 'red'],
];
async function loadSample() {
  const s = wkStart(new Date());
  for (const [o, title, type, loc, a, b, tech, status, color] of SAMPLE) {
    const r = { uuid: uuid(), title, type, calendar: 'my', location: loc, technician: tech, status, color, notes: '', all_day: a ? 0 : 1,
      start_at: ymd(addD(s, o)) + ' ' + (a || '00:00') + ':00', end_at: ymd(addD(s, o)) + ' ' + (b || '23:59') + ':00', updated_at: Date.now(), dirty: 1, deleted: 0 };
    st.events.set(r.uuid, r); await put(r);
  }
  render(); sync();
}
...
f.addEventListener('submit', async (ev) => {
  ...
  const rec = Object.assign({}, id ? st.events.get(id) : {}, {
    uuid: id || uuid(), title, type: F('type').value, calendar: F('calendar').value, location: F('location').value.trim(), technician: F('technician').value.trim(),
...
function exportCsv() {
  ...
  const csv = ['Date', 'Start', 'End', 'Title', 'Type', 'Location', 'Technician', 'Status'].join(',') + '\n' +
...
const pub = (r) => ({ uuid: r.uuid, title: r.title, type: r.type, calendar: r.calendar, location: r.location, technician: r.technician, status: r.status,
  start_at: r.start_at, end_at: r.end_at, all_day: r.all_day ? 1 : 0, color: r.color || null, notes: r.notes || null, updated_at: r.updated_at, deleted: r.deleted ? 1 : 0 });

async function refreshCsrf() {
  const r = await fetch(S.urls.events, { credentials: 'same-origin', headers: { Accept: 'application/json' } });
  if (r.status === 401 || r.status === 403) throw new Error('auth');
  S.csrf = (await r.json()).csrf;
}

async function exchange(batch) {
  const send = () => fetch(S.urls.sync, { method: 'POST', credentials: 'same-origin',
    headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'X-CSRF-TOKEN': S.csrf, 'X-Requested-With': 'XMLHttpRequest' },
    body: JSON.stringify({ changes: batch.map(pub) }) });
  let r = await send();
  if (r.status === 419) { await refreshCsrf(); r = await send(); }
  if (r.status === 401 || r.status === 403) throw new Error('auth');
  if (!r.ok) throw new Error('http ' + r.status);
  const j = await r.json(); S.csrf = j.csrf || S.csrf; return j;
}

async function merge(res, sent) {
  let changed = false;
  for (const p of sent) { // rows the server has now accepted
    const r = st.events.get(p.uuid);
    if (r && r.updated_at === p.updated_at) { if (r.deleted) { st.events.delete(p.uuid); await del(p.uuid); } else { r.dirty = 0; r.synced = 1; await put(r); } }
  }
  const onServer = new Set();
  for (const s of res.events) {
    onServer.add(s.uuid);
    const l = st.events.get(s.uuid);
    if (l && l.dirty) continue; // an edit made while the request was in flight stays; it goes out next round
    if (!l || l.updated_at !== s.updated_at) changed = true;
    const rec = Object.assign({}, s, { dirty: 0, synced: 1, deleted: 0 });
    st.events.set(s.uuid, rec); await put(rec);
  }
  for (const [id, l] of [...st.events]) { // removed on the server (e.g. from another device)
    if (l.synced && !l.dirty && !onServer.has(id)) { st.events.delete(id); await del(id); changed = true; }
  }
  return changed;
}
...
async function sync() {
  ...
  if (st.syncing) { st.again = true; return; }
  st.syncing = true; st.err = ''; pill();
  let changed = !st.ready;
  try {
    const dirty = () => [...st.events.values()].filter((e) => e.dirty);
    let pending = dirty(), guard = 0;
    do {
      const batch = pending.slice(0, 100);
      if (await merge(await exchange(batch), batch)) changed = true;
      pending = dirty();
    } while (pending.length && ++guard < 20);
  } catch (e) {
    if (e.message === 'auth') st.err = 'Sign in again to sync';
  } finally {
    st.syncing = false; st.ready = true;
    if (changed) render(); else pill();
    if (st.again) { st.again = false; sync(); }
  }
}
...
document.addEventListener('click', (e) => {
  ...
  if (mi) { const id = menu.dataset.id; closeMenu(); if (mi.dataset.act === 'edit') openForm(id); else if (confirm('Delete this schedule?')) removeEv(id); return; }
...
$('#btnDel').onclick = () => { const id = f.dataset.id; if (id && confirm('Delete this schedule?')) { closeForm(); removeEv(id); } };
...
(async () => {
  render();
  try { await idbOpen(); (await idbAll()).forEach((r) => st.events.set(r.uuid, r)); }
  catch (e) { console.warn('Offline storage is not available in this browser; changes will only be kept while online.', e); }
  st.ready = !navigator.onLine || st.events.size > 0;
  render();
  sync();
})();
})();
