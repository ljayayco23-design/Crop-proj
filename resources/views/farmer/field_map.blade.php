...
<script>
    tailwind.config = {
        ...
        theme: {
            extend: {
                colors: { darkBg: '#0e1116', panelBg: '#161b22', panelBorder: '#30363d', accent: '#2f81f7', accentHover: '#1f6feb' },
...
.leaflet-pm-toolbar .active { background-color: #2f81f7 !important; color: #ffffff !important; }
...
.export-bounding-outline { border: 2px dashed rgba(47, 129, 247, 0.6); box-shadow: none; pointer-events: none; }
...
<div class="container-fluid py-4">
    <div class="row mb-3">
        <div class="col-12">
            <h2 class="fw-bold text-success mb-1"><i class="fas fa-map-location-dot me-2"></i> Field Intelligence</h2>
        ...
        <div class="flex flex-col lg:flex-row flex-1 overflow-y-auto lg:overflow-hidden">
            ...
            <main class="flex-1 flex flex-col bg-[#0e1116] min-w-0 min-h-[60vh] lg:min-h-0 relative z-0">
                ...
                <div class="bg-[#0e1116] border-t border-panelBorder grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-panelBorder shrink-0 z-20">
                    ...
                    <div class="p-5 flex flex-col justify-between transition-opacity duration-300" id="properties-panel" style="opacity: 0.5; pointer-events: none;">
                        <div id="shape-props">
                            ...
                            <div class="grid grid-cols-2 gap-x-4 gap-y-2">
                                <div class="flex justify-between items-center"><label class="text-xs text-gray-400">Fill Color</label><div class="flex items-center gap-1.5"><input type="color" id="prop-fill-color" value="#3882F6" class="w-5 h-5 rounded cursor-pointer bg-transparent border-0 p-0" onchange="updateSelectedShape()"><span class="text-[11px] text-gray-400 font-mono" id="prop-fill-hex">#3882F6</span></div></div>
                                <div class="flex justify-between items-center"><label class="text-xs text-gray-400">Border Color</label><div class="flex items-center gap-1.5"><input type="color" id="prop-border-color" value="#3882F6" class="w-5 h-5 rounded cursor-pointer bg-transparent border-0 p-0" onchange="updateSelectedShape()"><span class="text-[11px] text-gray-400 font-mono" id="prop-border-hex">#3882F6</span></div></div>
                    ...
                    <div class="p-5 flex flex-col justify-between">
                        ...
                        <button id="btn-export-img" onclick="exportMapImage()" class="w-full bg-[#238636] hover:bg-[#2ea043] text-white py-2 rounded-md font-medium text-xs flex items-center justify-center gap-2 transition-colors"><i class="ph ph-download-simple text-lg"></i> Export Render Snapshot</button>
                    </div>
                </div>
            </main>
            ...
            <aside class="w-full lg:w-[320px] border-t lg:border-t-0 lg:border-l border-panelBorder bg-panelBg flex flex-col shrink-0 z-10 overflow-hidden shadow-xl">
                ...
                <div class="p-4 flex-1 flex flex-col min-h-0 bg-panelBg overflow-hidden map-container-wrapper">
                    <!-- Added specific container heights and scrollbar to measurements -->
                    <div class="mb-6 flex flex-col max-h-[50%]">
                        <div class="flex justify-between items-center mb-3 border-b border-panelBorder pb-2 shrink-0">
                            <div>
                                <h2 class="text-xs font-semibold text-gray-200 uppercase tracking-wider">Your Measurements</h2>
                    ...
                    <!-- Added specific container heights and scrollbar to other fields -->
                    <div class="flex flex-col max-h-[50%]">
                        <div class="flex justify-between items-center mb-3 border-b border-panelBorder pb-2 shrink-0">
                            <div>
                                <h2 class="text-xs font-semibold text-[#38bdf8] uppercase tracking-wider"><i class="ph-fill ph-users"></i> Other Fields</h2>
                            </div>
                        </div>
...
const SYNC_URL = '{{ route("farmer.field_map.sync") }}';
const WEATHER_URL = '{{ route("farmer.field_map.weather") }}';
...
function updateLayersList() {
    ...
    layers.forEach(layer => {
        ...
        measurementsHTML += `
            <div class="bg-darkBg p-3 rounded-lg border ${isSelected ? 'border-accent shadow-[0_0_10px_rgba(47,129,247,0.3)]' : 'border-panelBorder hover:border-gray-500'} cursor-pointer transition group" onclick="selectLayerById(${L.stamp(layer)})">
